"""
Add CV features on manager, add "small counts" to large counts
"""
import argparse
import logging
import pdb
...
def compute_manager_cv_features(dev_df, val_df):
    ...
    for agg_feat in ['listing_id', 'bedrooms', 'bathrooms', 'rooms', 'price', 'price_per_room', 'price_per_bed',
                     'price_per_bath']:
        df = manager_interest_aggs[agg_feat]
        for level in ['low', 'medium', 'high']:
            fname = 'manager_%s_agg_%s' % (level[0], agg_feat)
