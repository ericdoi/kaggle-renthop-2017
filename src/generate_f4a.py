...
def add_small_count_features(train_df, test_df, cat_cols, max_obs=10):
    cat_df = pd.concat([train_df, test_df], axis=0)[cat_cols]
...
def add_cv_features(train_df, test_df, n_folds=N_FOLDS_CV_FEATS):
    """
    ...
    for k, (dev_ix, val_ix) in enumerate(fold_gen.split(train_df.values)):
        ...
        fold_aggs_df, _ = compute_manager_cv_features(dev_df, val_df)
        aggs_df_list.append(fold_aggs_df)

    # Vertically stack the fold rows
    train_aggs_df = pd.concat(aggs_df_list, axis=0)
    test_aggs_df, added_cv_feats = compute_manager_cv_features(train_df, test_df)
...
def compute_manager_cv_features(dev_df, val_df):
    ...
    """
    manager_interest_aggs = dev_df.groupby(['manager_id', 'interest_level']).agg({
        'listing_id': 'count',
        'price': 'mean',
        'bedrooms': 'mean',
        'bathrooms': 'mean',
    ...
    manager_interest_aggs = manager_interest_aggs.unstack()
    ...
    df = manager_interest_aggs['listing_id']
    manager_interest_aggs['manager_skill'] = (df['high'] * 2 + df['medium']) / (df.sum(axis=1))

    # Rename other aggregation features
    added_cv_feats = ['manager_skill']
    for agg_feat in ['bedrooms', 'bathrooms', 'rooms', 'price']:
        df = manager_interest_aggs[agg_feat]
        for level in ['low', 'medium', 'high']:
            fname = 'manager_%s_avg_%s' % (level[0], agg_feat)
            manager_interest_aggs[fname] = df[level]
            added_cv_feats.append(fname)

    manager_interest_aggs = manager_interest_aggs[added_cv_feats]  # Drop other columns
    manager_interest_aggs.columns = manager_interest_aggs.columns.get_level_values(0)  # De-nest columns

    # Join with train_df indices
    aggs_df = pd.merge(
        val_df[['manager_id']],
        manager_interest_aggs,
        left_on='manager_id',
        right_index=True
    )
...
def make_text_features(train_df, test_df):
    ...
    ctr = CountVectorizer(stop_words='english', max_features=200, vocabulary=keep_tags)
...
def run(train_file, test_file, train_feature_file, test_feature_file, feature_map_file,
    ...
    train_df, test_df, added_cv_feats = add_cv_features(train_df, test_df)
    ...
    features_to_use = numeric_features + added_cv_feats + added_features + categorical_features + count_feature_names
