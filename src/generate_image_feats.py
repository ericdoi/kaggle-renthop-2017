...
def aggregate_image_feats(df):
    ...
    agg_dict['ct_portrait_top2'] = top2_df['portrait'].sum()
    ...
    agg_dict['min_img_y'] = df['img_y'].min()
    agg_dict['min_img_i'] = df['img_i'].min()
    agg_dict['min_img_q'] = df['img_q'].min()
    agg_dict['max_img_y'] = df['img_y'].max()
    agg_dict['max_img_i'] = df['img_i'].max()
    agg_dict['max_img_q'] = df['img_q'].max()

    agg_dict['avg_filesize'] = df['filesize'].mean()
    agg_dict['min_filesize'] = df['filesize'].min()
    agg_dict['max_filesize'] = df['filesize'].max()

    agg_dict['avg_histogram_skew'] = df['histogram_skew'].mean()
    agg_dict['min_histogram_skew'] = df['histogram_skew'].min()
    agg_dict['max_histogram_skew'] = df['histogram_skew'].max()

    agg_dict['sum_ocr_len'] = df['ocr_words'].apply(lambda x: len(x)).sum()
    agg_dict['has_floorplan'] = df['is_floorplan'].sum()
    agg_dict['ct_portrait'] = df['portrait'].sum()
...
def main():
    ...
    parser.add_argument("--image-base-path", default='../data/images_sample/', help="path to photos")
